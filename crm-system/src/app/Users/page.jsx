...
const Users = () => {
  ...
  const handleEditUser = (user) => {
    setIsEditing(true);
    setEditUser({ ...user, password: "" }); // Initialize password as empty
    setError(null); // Reset error state
  };
  ...
  const handleUpdateSubmit = (e) => {
    ...
    axios.put(`/api/users/${editUser._id}`, editUser)
      .then((response) => {
        if (response.data.success && response.data.data) {
          setUsers((prevUsers) =>
            prevUsers.map((user) =>
                  ...
                  {users.length > 0 ? (
                    users.map((user) => (
                      <tr key={user._id} className="hover:bg-gray-50">
                        <td className="py-2 border-b text-center">{user.role}</td>
                        <td className="py-2 border-b text-center">{user.firstName}</td>
                        <td className="py-2 border-b text-center">{user.lastName}</td>
                        <td className="py-2 border-b text-center">{user.phoneNumber}</td>
                        <td className="py-2 border-b text-center">{user.email}</td>
                        <td className="py-2 border-b text-center">{new Date(user.dateOfBirth).toLocaleDateString()}</td>
                        <td className="py-2 border-b text-center">{user.position}</td>
                        <td className="py-2 border-b text-center">{user.location}</td>
