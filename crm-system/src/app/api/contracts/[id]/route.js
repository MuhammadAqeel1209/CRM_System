...
import Contracts from '@/app/Model/Contract'; 
...
export async function PUT(request, { params }) {
  ...
  console.log(id)
  try {
    let contract = await Contracts.findByIdAndUpdate(id, data);
...
export async function GET(request, { params }) {
  ...
  await connectDatabase();
  try {
    const table = await Contracts.findOne({ _id: id });
    if (!table) {
...
export async function DELETE(request, { params }) {
  ...
  await connectDatabase();
  try {
    const deletedRecord = await Contracts.findByIdAndDelete(id);
    if (!deletedRecord) {
      return NextResponse.error(new Error("Record not found"), { status: 404 });
    }
